...
def test_find_one_include_deleted(
    ...
    with organization_repository.db_session:
        ...
        actual = organization_repository.find_one(organization_entity.id, include_deleted=True)
...
def test_find_should_return_by_external_id(
    organization_repository: OrganizationRepository,
    organization_entity: OrganizationEntity,
) -> None:
    with organization_repository.db_session:
...
def test_find_should_only_load_what_is_in_context(
    organization_repository: OrganizationRepository,
    organization_entity: OrganizationEntity,
    client_entity: ClientEntity,
) -> None:
...
def test_find_should_include_deleted(
    ...
    with organization_repository.db_session:
        ...
        ctx = OrganizationQueryContext(external_id=organization_entity.external_id)
        result = organization_repository.find(ctx, include_deleted=True)
...
def test_find_many_filters_by_external_id(
    organization_repository: OrganizationRepository,
    organization_entity: OrganizationEntity,
) -> None:
    entity_2 = OrganizationEntity(external_id="87654321", name="Another Organization")
...
def test_find_many_should_filter_on_children(
    ...
    org_entity_2 = OrganizationEntity(external_id=UraNumber("90001002"), name="Test Org 2")
    with organization_repository.db_session as session:
        organization_entity.clients.append(client_entity)
        organization_entity.certificates.append(certificate_entity)
        ...
        org_entity_2.clients.append(ClientEntity(name="Other Test Client"))
        org_entity_2.certificates.append(
            CertificateEntity(organization_identifier=Oin("00000099000000002000"), domain="some-other-domain")
        )
        org_entity_2.sources.append(SourceEntity(source_id="source-id-2", name="some-other-source"))
        org_2 = organization_repository.add_one(org_entity_2)

        certificate_entity_2 = org_2.certificates[0]
        client_entity_2 = org_2.clients[0]
        source_entity_2 = org_2.sources[0]
        session.session.flush()
        session.session.expire_all()
